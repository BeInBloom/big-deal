// Code generated by BobGen psql v0.46.0. DO NOT EDIT.
// This file is meant to be re-generated in place and/or deleted at any time.

package factory

import "context"

type contextKey string

var (
	factoryVisitedCtx = newContextual[map[uintptr]struct{}]("factoryVisited")

	// Relationship Contexts for order_parts
	orderPartWithParentsCascadingCtx = newContextual[bool]("orderPartWithParentsCascading")
	orderPartRelOrderCtx             = newContextual[bool]("order_parts.orders.order_parts.order_parts_order_id_fkey")

	// Relationship Contexts for orders
	orderWithParentsCascadingCtx = newContextual[bool]("orderWithParentsCascading")
	orderRelOrderPartsCtx        = newContextual[bool]("order_parts.orders.order_parts.order_parts_order_id_fkey")

	modelsInCreationCtx = newContextual[map[string]any]("modelsInCreation")
)

// Contextual is a convienience wrapper around context.WithValue and context.Value
type contextual[V any] struct {
	key contextKey
}

func newContextual[V any](key string) contextual[V] {
	return contextual[V]{key: contextKey(key)}
}

func (k contextual[V]) WithValue(ctx context.Context, val V) context.Context {
	return context.WithValue(ctx, k.key, val)
}

func (k contextual[V]) Value(ctx context.Context) (V, bool) {
	v, ok := ctx.Value(k.key).(V)
	return v, ok
}
